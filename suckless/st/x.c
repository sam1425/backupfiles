...
xhints(void)
{
	XClassHint class = {opt_name ? opt_name : stclass,
	                    opt_class ? opt_class : stclass};
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int charlen, int dmode)
{
	...
	if (dmode & DRAW_BG) {
		...
		XftDrawRect(xw.draw, bg, winx, winy + 0.5f, width, win.ch);
	}
	...
	if (base.mode & ATTR_UNDERLINE) {
		...
		/*8int liney = winy + dc.font.ascent + gap*/
